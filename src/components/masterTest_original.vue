          ...
          <div v-if="qnumber === index" :class="[ qnumber===0 || qnumber===1 ? 'question-title type1' : 'question-title']">
            ...
            <ol class="answers" :class="[ qnumber===2 || qnumber===3 ? 'type2' : [ qnumber===4 || qnumber===5 || qnumber===6 || qnumber===7 || qnumber===8 || qnumber===9 || qnumber===10 ? 'type3' : [ qnumber===11 ? 'type4' : 'type1'] ] ]">
              ...
              <li v-if="item.multi === undefined" v-for="q in item.question" v-on:click="singleSelected(index, q.id)" :class="[ q.id === selected['q'+(index+1)]===true ? 'highlight' : [ q.selected!==undefined && q.selected===false ? 'hold' : '' ]]">
                <div v-if="q.imageSrc" class="img-clip" :class="[ q.selected!==undefined && q.selected===false ? 'hold' : '' ]">
                  <img :src="q.imageSrc">
                </div>
                <p v-html="q.q"></p>
              </li>
  ...
  export default {
    ...
    methods: {
      singleSelected: function(index, id){
        this.isNext = true;
        this.selected['q'+(index+1)] = id;
      },
      multiSelected: function(index,id){

        if(this.items[index+1] !== undefined && this.items[index+1].question.filter(item => item.id === id)[0].selected !== undefined){
          this.items[index+1].question.filter(item => item.id === id)[0].selected = !this.items[index+1].question.filter(item => item.id === id)[0].selected;
        }

        if(id === 0){
          this.selected['q'+(index+1)] = [];
          this.selected['q'+(index+1)].push(0);
          if(this.qnumber !== 8 && this.qnumber !== 11) {
            for(var i=0; i<this.items[index+1].question.length; i++){
              this.items[index+1].question[i].selected = false;
            }
          }
        }else if(this.selected['q'+(index+1)].includes(0)){
          this.selected['q'+(index+1)] = [];
          this.selected['q'+(index+1)].push(id);

        }else{
          if(this.selected['q'+(index+1)].includes(id)){
            this.selected['q'+(index+1)].splice(this.selected['q'+(index+1)].indexOf(id), 1);
            if(this.selected['q'+(index+1)].length<=0){
              this.isNext = false;
            }
          }else{
            this.selected['q'+(index+1)].push(id);
      ...
      prevStep: function(){
        if(this.qnumber === 6 && this.selected['q6'].length === 0 || this.qnumber === 8 && this.selected['q8'].length === 0){
          this.qnumber -= 2;
          this.isNext = true;
        }else{
          this.qnumber -= 1;
      ...
      nextStep: function(){
        ...
        if(this.qnumber === 4 && this.selected.q5.length<=1){
          if(this.selected.q5.includes(0)){
            this.selected.q6 = [];
          }
          this.qnumber += 2;
          this.$eventHub.$emit('progressDatas',{
            'currentStep': this.qnumber
          });

        }else if(this.qnumber === 6 && this.selected.q7.length<=1){
          if(this.selected.q7.includes(0)){
            this.selected.q8 = [];
          }
          this.qnumber += 2;
          this.$eventHub.$emit('progressDatas',{
          ...
          if(this.qnumber === 1 ){
            var idx = this.selected['q'+(this.qnumber+1)];
            var q = this.items[1].question.filter(item => item.id === idx)[0];
          }else if(this.qnumber === 3 ){
            var idx = this.selected['q'+(this.qnumber+1)];
            var q = this.items[3].question.filter(item => item.id === idx)[0];
          }else if(this.qnumber === 11){
            this.$eventHub.$emit('selectedDatas',{
        ...
        if(this.qnumber !== 12 && this.selected['q'+(this.qnumber+1)].length<=0){
          this.isNext = false;
        }else{
          this.isNext = true;
        }
